...
begin
    ...
    Inc(FClientNum);
    lClient                 := FClientClass.Create(Self);
    TriggerClientCreate(lClient);
    lClient.Name            := Name + 'Client' + IntToStr(FClientNum);
    lClient.Banner          := FBanner;
    lClient.Server          := Self;
    lClient.HSocket         := Accept;
    TriggerClientConnect(lClient, Error);
    { The event handler may have destroyed the client ! }
    if FClientList.IndexOf(lClient) < 0 then
        Exit;
    { The event handler may have closed the connection }
    if lClient.State <> wsConnected then
        Exit;
    ...
    if (FMaxClients > 0) and (FMaxClients < ClientCount) then begin
        ...
        lClient.Banner := FBannerToBusy;
        lClient.StartConnection;
        lClient.Close;
    end
    else
        lClient.StartConnection;
...
var
    lClient : TWSocketClient;
begin
   lClient := TWSocketClient(Msg.LParam);
    try
        TriggerClientDisconnect(lClient, Error);
    finally

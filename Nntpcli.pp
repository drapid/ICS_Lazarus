...
uses
    Windows, SysUtils, Classes, WinSock, WSocket;
...
{$ENDIF}
        ...
        function NTPCliAllocateHWnd(Method: TWndMethod) : HWND;
        procedure NTPCliDeallocateHWnd(WHandle : HWND);
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TNntpCli.NTPCliAllocateHWnd(Method: TWndMethod) : HWND;
begin
//{$IFDEF NOFORMS}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TNntpCli.NTPCliDeallocateHWnd(WHandle : HWND);
begin
{$IFDEF NOFORMS}
...
{$ENDIF}
    FWindowHandle               := NTPcliAllocateHWnd(WndProc);
...
{$ENDIF}
    if Assigned(FWSocket) then
        FWSocket.Destroy;
    NTPCliDeallocateHWnd(FWindowHandle);
    inherited Destroy;
end;

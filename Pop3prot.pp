...
type
    ...
    protected
        ...
        procedure   RetrDone;
        procedure   LastDone;
        procedure   Pop3CliWndProc(var MsgRec: TMessage); virtual;
    	...
    	function    Pop3CliAllocateHWnd(Method: TWndMethod): HWND; virtual;
    	procedure   Pop3CliDeallocateHWnd(WHandle: HWND); virtual;
...
//{$IFDEF NOFORMS}
    ...
    SetWindowLong(Result, GWL_WNDPROC, LongInt( Pop3CliWndProc ));
//{$ELSE}
...
begin
    ...
    FWindowHandle            := Pop3CliAllocateHWnd(Pop3CliWndProc);
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomPop3Cli.Pop3CliWndProc(var MsgRec: TMessage);
begin
...
begin
    ...
    while TRUE do begin
        ...
        if  ((FTimeout > 0) and (Integer(GetTickCount) > FTimeStop)) then begin

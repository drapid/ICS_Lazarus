...
Jul 31, 2001 V1.02 Handle URL encoded document file (for example when there is
             a space in the file name). Bug reported by Stian Fisk
             <joepezt@berzerk.net>.
...
const
    THttpServerVersion = 102;
    CopyRight : String = ' THttpServer (c) 1999-2001 F. Piette V1.02 ';
...
type
    ...
    public
        ...
        FConnectState                 : THttpConnectionState;
        FDocDir                : String;
        FDefaultDoc            : String;
        FDocument              : String;
        FDocStream             : TStream;
...
begin
    ...
    if FConnectState = hcPostedData then begin
    ...
    if FConnectState = hcRequest then begin
...
begin
    ...
    hgSendDoc:
        begin
            OK := FALSE;
            try
                if not FileExists(FDocument) then

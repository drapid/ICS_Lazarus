...
type
	...
	FOnPartEnd                : TNotifyEvent;
        ...
        function  UUProcessLine(aCurrentData: pchar): boolean;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TMimeDecode.UUProcessLine(aCurrentData: PChar): Boolean;
var
    s           : String;
    out1        : String;
    pos1        : Integer;
...
begin
    Result := TRUE;
{   if aCurrentData^ = #0 then begin
        result := false;
        Exit;
    end; }
    s := Trim(StrPas(aCurrentData));
    if S = '' then begin
        Result := FALSE;
        Exit;
    end;

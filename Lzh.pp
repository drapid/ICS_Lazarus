...
TYPE
  ...
  { pointing children nodes (son[], son[] + 1)}
    ...
    Procedure LZHUnpack(aTextSize : Longint; GetBytes:GetBytesProc;
                                            PutBytes: PutBytesProc);
      {#X LZHPack}
      {
...
VAR
   ct : BYTE;
   i, alen, r, s, last_match_length : Int16;
   Got : WORD;
BEGIN
...
try
  ...
  alen := 0;
  Got := 1;
  While (alen < F) AND (Got <> 0) DO BEGIN
    GetBytes(ct,1,Got);
    IF Got <> 0 THEN BEGIN
      text_buf^[r + alen] := ct;
      INC(alen);
  ...
  InsertNode(r);
  REPEAT
    IF (match_length > alen) THEN begin
      match_length := alen;
    end;
    ...
    While (i < last_match_length) DO BEGIN
      ...
      r := SUCC(r) AND PRED(N);
      DEC(alen);
      IF BOOLEAN(alen) THEN InsertNode(r);
    END; { endwhile }
...
Procedure TLZH.LZHUnpack(aTextSize : Longint; GetBytes:GetBytesProc; PutBytes: PutBytesProc);
VAR
  c, i, j, k, r : Int16;
  c2            : Byte;
  count         : Longint;
...
try
  ...
  FillChar(text_buf^[0],r,' ');
  Count := 0;
  While count < atextsize DO BEGIN
